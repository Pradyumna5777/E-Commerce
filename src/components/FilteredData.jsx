...
const FilteredData = () => {
    ...
    const {
      ...
      // console.log("homedataaa", homeData);
  ...
  return (
    ...
    <div className="min-h-[100vh] bg-white mt-[9vh] object-cover w-[100vw] flex gap-[4vh] justify-center py-[4vh] flex-wrap text-[#fff] relative">
      <div className="grid grid-cols-4 gap-[2vw] text-black transition-all ease-in-out">
        {filteredData.map((e, index) => {
          return (
            <div key={index}>
              <div className="md:min-h-[74vh] h-[45vh] shadow-xl flex flex-col items-center gap-[4vh] w-[48vw] md:mt-4 mt-6 md:w-[20vw] overflow-hidden relative border-[#0000001b] border-[1px]">
                <div
                  onClick={() => tileBtn(e, index)}
                  className="relative"
                >
                  {e.image ? (
                    <img
                      className="md:h-[32vh] h-[12vh] cursor-pointer md:w-[20vw] w-[100%] object-cover hover:scale-[1.1] hover:rounded-b-[6px] transition duration-300 ease-out"
                      src={e.image}
                    />
                  ) : (
                    <img
                ...
                <div className="flex flex-col w-full px-1 md:p-2 md:gap-y-[1vh] gap-y-[0.3vh]">
                  ...
                  {e.category ? (
                    <p className="font-semibold md:text-sm text-xs">
                      <b className="md:text-md text-sm">{e.category}</b>
                    </p>
                  ) : (
                    ""
                  )}
                  ...
                  {e.discount ? (
                    <p className="flex items-center gap-1 font-bold md:mt-4 mt-2 md:text-sm text-xs">
                      ...
                      ₹
                      {(e.price - (e.discount / 100) * e.price).toFixed(
                        2
                      )}
                    </p>
                  ) : (
                  ...
                  {e.rating && (
                    <p className="font-semibold md:text-sm text-xs">
                      <b className="md:text-md text-sm">Ratings:</b> ⭐
                      {e.rating.rate}/5
                    </p>
                  )}
                  ...
                  {e.color ? (
                    <p className="font-semibold md:text-sm text-xs">
                      <b className="md:text-md text-sm">Color:</b>{" "}
                      {e.color}
                    </p>
                  ) : (
                    ""
                  ...
                  {e.model ? (
                    <p className="font-semibold md:text-sm text-xs">
                      <b className="md:text-md text-sm">Model:</b>{" "}
                      {e.model.slice(0, 29)}
                    </p>
                  ) : (
                    ""
